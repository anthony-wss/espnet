...
if [ ! -e "${TW_PODCAST}" ]; then
    log "Fill the value of 'TW_PODCAST' of db.sh"
...
train_set=train #train_"$(echo "${set}" | tr "[:upper:]" "[:lower:]")"
dev_set=dev
test_sets=test #"test_net test_meeting"
